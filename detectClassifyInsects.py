...
def appendToCSV(orderClass, img, srcpath, name, imsize, cls, x, y, w, h, conf):
    ...
    orderClass.appendToBatch(imgCrop)
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    #orderClass = orderClassifier('./saved_resnet_128/dhc_best.pth', './saved_resnet_128/thresholdsTestTrain.csv', 'cuda:0', 128)
    orderClass = orderClassifier(resnet, labels, 'cuda:0', 128)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                orderClass.createBatch(len(det))                
                csvLines = []
                # Write results
                for *xyxy, conf, cls in reversed(det):
                    ...
                    filename = str(p.name)
                    im0size = im0.shape
                    line = appendToCSV(orderClass, im0, source, filename, im0size, cls, *xywh, conf)
                ...
                linePredictions, predictions = orderClass.classifyBatch()
                
                dstCSVfile = result + '.csv'
                for idx in range(len(csvLines)):
                    line = csvLines[idx] + ',' + linePredictions[idx] + ',' + str(keyIdx) + '\n'
