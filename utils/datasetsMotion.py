...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, motion='none'): # 'none', 'RGB', 'HSV'
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            img1 = cv2.imread(path)  # BGR
            #print("Motion ", path)
            if self.motionMethod == 'none':
                img0 = img1
            else:
                img0 = self.motion_image(img1, method=self.motionMethod)
        ...
        if self.motionMethod == 'none':
            ...
            # Motion uses previous image
            if self.pathPrev is None:
                self.pathPrev = path
            prevPath = self.pathPrev
            self.pathPrev = path

        return prevPath, img, img0, self.cap, s
