...
def appendToCSV(orderSpeciesClass, img, srcpath, name, imsize, cls, x, y, w, h, conf):
    ...
    orderSpeciesClass.appendToBatch(imgCrop)
    
    # Saved crops in files
    #cropFileName = './Crops/'+name
    #cv2.imwrite('./Crops/'+name, imgCrop)
    #print("Saved insect crop", cropFileName)
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    orderSpeciesClass = orderSpeciesClassifier(species_gbif_model, order_model, order_labels, species_model, species_labels, device.type, 128)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                orderSpeciesClass.createBatch(len(det))                
                csvLines = []
                # Write results
                ...
                lineSpeciesPredictions, _ = orderSpeciesClass.classifySpeciesTrapBatch()
...
def parse_opt():
    ...
    parser.add_argument('--order-model', type=str, default='./model_order_100524/dhc_best_128.pth', help='Resnet weights for oder classifier')
    parser.add_argument('--order-labels', type=str, default='./model_order_100524/thresholdsTestTrain.csv', help='CSV file with order labels and thresholds')
    parser.add_argument('--species-model', type=str, default='./model_species_251224/dhc_best_128.pth', help='Resnet weights for species classifier')
    parser.add_argument('--species-labels', type=str, default='./model_species_251224/thresholdsTestTrain.csv', help='CSV file with species labels and thresholds')
    parser.add_argument('--species-gbif-model', type=str, default='./ami', help='Path to save and load AMI moth species classifier')
