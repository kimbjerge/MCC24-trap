...
#KBE??? from trapdata import constants, logger
#KBE??? from trapdata.db.models.detections import save_classified_objects
#KBE??? from trapdata.db.models.queue import DetectedObjectQueue, UnclassifiedObjectQueue
...
class BinaryClassifier(EfficientNetClassifier):
    ...
    #KBE??? def get_queue(self) -> DetectedObjectQueue:
    #KBE???     return DetectedObjectQueue(self.db_path, self.image_base_path)
        ...
        classified_objects_data = [
            {
                ...
                #KBE??? "in_queue": True if label == constants.POSITIVE_BINARY_LABEL else False,
